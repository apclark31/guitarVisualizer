...
export const useMusicStore = create<AppState>((set, get) => ({
  ...
  clearAllStrings: () => {
    // Clear key context in shared store
    getSharedState().setKeyContext(null);
  ...
  restoreFromUrl: (params) => {
    ...
    // Update shared store with restored tuning and key context
    sharedState.setTuning([...effectiveTuning], effectiveTuningName);
    sharedState.setKeyContext(keyContext ?? null);
