...
beforeEach(() => {
  useMusicStore.setState({
    ...
    voicingTypeFilter: 'all', displayMode: 'notes', isCustomShape: false,
  });
  useSharedStore.getState().setTuning([...STANDARD_TUNING], 'Standard');
  useSharedStore.getState().setKeyContext(null);
});
...
describe('useMusicStore', () => {
  ...
  describe('clearAllStrings', () => {
    it('resets all fretboard and detection state', () => {
      // Set up state
      state().setTargetChord('G', 'Major');
      useSharedStore.getState().setKeyContext({ root: 'C', type: 'major' });
      ...
      expect(useSharedStore.getState().keyContext).toBeNull();
  ...
  describe('restoreFromUrl', () => {
    ...
    it('restores keyContext when provided', () => {
      ...
      expect(useSharedStore.getState().keyContext).toEqual({ root: 'C', type: 'major' });
    });
  ...
  // ── setKeyContext (shared store) ──────────────────────────────────

  describe('setKeyContext', () => {
    const shared = () => useSharedStore.getState();

    it('sets key context in shared store', () => {
      shared().setKeyContext({ root: 'G', type: 'major' });
      expect(shared().keyContext).toEqual({ root: 'G', type: 'major' });
    });

    it('clears key context with null', () => {
      shared().setKeyContext({ root: 'A', type: 'minor' });
      shared().setKeyContext(null);
      expect(shared().keyContext).toBeNull();
    });
  });
